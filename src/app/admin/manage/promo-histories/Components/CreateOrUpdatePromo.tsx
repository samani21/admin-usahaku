...
export default function CreateOrUpdatePromo({ onClose, editPromo }: Props) {
    ...
    const getProduct = async () => {
        try {
            const res = await Get<{ success: boolean, data: ItemProduct[] }>('/promo/get-products')
            if (res?.success) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = editPromo ? `promo/${editPromo?.id}` : 'promo'
                                                            ...
                                                            <div className="flex flex-col items-end gap-1" onClick={e => e.stopPropagation()}>
                                                                <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-lg border border-slate-200">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        placeholder="0"
                                                                        className="w-16 py-1 px-2 text-xs font-bold text-right bg-white border border-slate-200 rounded-md outline-none focus:border-emerald-500"
                                                                    />
                                                                </div>
