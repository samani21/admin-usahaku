...
import ServiceFormModalContent from "./ServiceFormModalContent";
...
export default function ListServicePage() {
    ...
    const fetchProducts = useCallback(async () => {
        ...
        setError('');
        try {
            const res = await Get<{ success: boolean; data: ProductResponse; meta: Meta }>(`/services${queryString}`);
            if (res?.success) {
    ...
    const onDelete = async (id: number | null) => {
        setLoading(true);
        try {
            const res = await Delete(`/services/${id}`);
            if (res) {
                fetchProducts();
                handleCloseModal();
    ...
    const columns: Column<ProductsType>[] = useMemo(() => [
        {
            ...
            render: (row) => {
                ...
                if (row.image.startsWith("usahaku")) {
                    return (
