...
import {
    AlertTriangle, // Tambahan icon untuk tampilan error
    ArrowLeft,
    ArrowRight,
    Building,
    Check,
...
function ResetView({ themeStyles, showToast, activeScheme, theme }: Props) {
    ...
    // State baru untuk menangani status Guard dari backend
    ...
    useEffect(() => {
        const validateToken = async () => {
            try {
                // Asumsi Anda mengambil token dari URL, misalnya: ?token=xxxxxx
                const urlParams = new URLSearchParams(window.location.search);
                const token = urlParams.get('token');
                console.log('token', token)
                if (!token) {
                    setTokenError('Link reset password tidak valid atau tidak ditemukan');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await Post('auth/reset-password', formData)
            showToast('Kata sandi berhasil diperbarui!', 'success');
            setTimeout(() => {
                window.location.href = '/auth/login'; // Redirect setelah sukses
            }, 1500);
        } catch (error: any) {
    ...
    // Tampilan Error jika Guard 1, 2, atau 3 tertangkap (Token tidak valid/kedaluwarsa/sudah dipakai)
    if (tokenError) {
        return (
    ...
    // Tampilan Form Reset (Akan muncul jika Guard 4 sukses / Token Valid)
    return (
        <div className='h-full flex flex-col justify-center'>
            <div className="animate-fadeIn">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                        type="submit"
                        disabled={isLoading || (form.newPassword !== form.confirmPassword) || form.newPassword === ''}
