            ...
            {products?.map((p, i) => {
                ...
                return (
                    <div
                        ...
                        className={`group relative p-1.5 rounded-[2.5rem] transition-all duration-700 ease-out overflow-hidden h-[380px] sm:h-[420px] flex flex-col shadow-lg
                            ${is_available
                                ? "bg-gradient-to-br from-indigo-500 via-fuchsia-500 to-amber-400 cursor-pointer hover:shadow-[0_20px_40px_rgba(0,0,0,0.2)] hover:-translate-y-2"
                                : "bg-slate-300 cursor-not-allowed opacity-80 grayscale"}`}
                    >
                        {/* Animated Gradient Glow Effect */}
                        {is_available && (
                            <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000 transform group-hover:translate-x-full" />
                        )}

                        {/* Top Badges */}
                        <div className="absolute top-5 left-5 right-5 flex justify-between z-20 pointer-events-none">
                            {label && is_available ? (
                                <div className="bg-white/20 backdrop-blur-md border border-white/40 text-white px-3 py-1.5 rounded-full font-black uppercase tracking-widest text-[9px] shadow-lg flex items-center gap-1.5">
                        ...
                        {/* Inner Glass Card */}
                        <div className={`h-full w-full rounded-[2rem] p-4 flex flex-col justify-between transition-colors duration-500 relative z-10 backdrop-blur-xl border
                            ${isDarkMode ? 'bg-[#0f0f11]/90 border-white/10 text-white' : 'bg-white/90 border-white/50 text-slate-900'}
                            ${!is_available ? 'opacity-80' : ''}`}>

                            {/* Image Container */}
                            <div className={`relative w-full h-44 shrink-0 rounded-[1.5rem] overflow-hidden transition-all duration-500 shadow-inner
                                ...
                                {is_available && (
                                    <div className="absolute bottom-3 right-3 w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-4 group-hover:translate-y-0">
                                ...
                                <div className="flex items-end justify-between mt-2 pt-3 border-t border-dashed border-current/10">
                                    ...
                                    {is_available && (
                                        <div className={`p-2.5 rounded-xl transition-colors duration-300
                                            ${isDarkMode ? 'bg-white/10 text-white group-hover:bg-indigo-500' : 'bg-slate-100 text-slate-900 group-hover:bg-indigo-500 group-hover:text-white'}`}>
